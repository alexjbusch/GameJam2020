...
function create_enemy(xin,yin,class_in,target_in,dir)
enemy = {
x=xin,
y=yin,
class=class_in,
...
init=function(self)
 ...
 elseif self.class == "thorns" then
  if (self.thorn_dir=="r") then
   self.sprite = 150
   self.thorn_fx = true
  end
  if (self.thorn_dir=="l") then
   self.sprite = 150
   self.thorn_fx = false
  end
  if (self.thorn_dir=="u") then
   self.sprite = 151
   self.thorn_fy = true
  end
  if (self.thorn_dir=="d") then
   self.sprite = 151
   self.thorn_fy = false
  end
 end
...
update=function(self)
 ...
 if (self.sp ~= nil) self:animate()
end,

draw=function(self)
 if self.class~="thorns" then
  spr(self.sprite,self.x,self.y)
 else
  spr(self.sprite,self.x,self.y,1,1,self.thorn_fx,self.thorn_fy)
 end
end,
...
stab_outwards=function(self)
 if self.state=="attacking" then
  if distance(player.x+4,player.y+4,self.x+4,self.y+4) <= 15 then
   player.hp-=1
  end
  create_enemy(self.x+8,self.y,"thorns",player,"r")
  enemy:init()
  create_enemy(self.x-8,self.y,"thorns",player,"l")
  enemy:init()
  create_enemy(self.x,self.y+8,"thorns",player,"u")
  enemy:init()
  create_enemy(self.x,self.y-8,"thorns",player,"d")
  enemy:init()
  self.attack_cooldown_timer=0
  self.state="idle"
 end
